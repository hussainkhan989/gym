...
class tricep extends StatefulWidget {
  const tricep({super.key});

  @override
  State<tricep> createState() => _tricepsState();
}

class _tricepsState extends State<tricep> {
  DocumentSnapshot? userSnapshot;

  Stream? datastream;

  getalldata() async {
    datastream = await DatabaseMethods().getdetailsfortricep();
    setState(() {});
  }
                ...
                child: InkWell(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => ExerciseDisplayScreen(
                            videourl: videourl,
                            gif: gif,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text('triceps Workout'),
        ),
        body: Container(
          //color: Colors.black,
          child: Column(
            children: [Expanded(child: gymwidget())],
          ),
        ));
  }
}
